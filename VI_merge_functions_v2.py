...
def concatenate_all_issues(vi):
  ...
  vi.loc[vi['all VI issues']!='', 'all VI issues'] = vi.loc[vi['all VI issues']!='', 'all VI issues'].transform(lambda x: ''.join(set(list(x))))
...
def print_conflict(vi,unique_targets,conflict_id):
  ...
  print(vi[vi.TARGETID==unique_targets[conflict_id]][['TARGETID','Redrock_spectype','VI_spectype','best spectype','Redrock_z','VI_z','best z','VI_quality','best quality','VI_issue','all VI issues','VI_comment','merger comment','VI_scanner']])
